...
export class SearchEngineCreateCollectionsService {
  ...
  async main(
    ...
    const aggregateCollections = payload.map((collection) =>
    ...
    // insert
    await this.repository.insert(aggregateCollections, {
    ...
    const collectionsRegistered = this.publisher.mergeObjectContext(
      new SearchEngineAddCollectionsContextEvent(aggregateCollections),
    );
