...
export async function runAuroraAgents(
  ...
  try {
    ...
    const runCtx = new RunContext<{ envelope: Partial<RequestEnvelope> }>({
      envelope: state,
    });
    // We'll reuse the same RunState across iterations
    let runState:
      | RunState<{ envelope: Partial<RequestEnvelope> }, Agent<any, any>>
      | undefined;
    ...
    while (true) {
      count++;
      if (count > 2)
        throw new Error('Too many iterations in agents orchestration loop');
      ...
      if (!runState) {
        ...
        runState = new RunState(
          runCtx,
          count === 1 ? userText : effectivePrompt,
          agent,
          3,
        );
      }
      ...
      console.log(
        'stepCurrentIndex:',
        stepCurrentIndex + 1 >= defaultFlow.length,
      );
