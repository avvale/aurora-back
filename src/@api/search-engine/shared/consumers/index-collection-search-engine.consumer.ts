...
export class IndexCollectionSearchEngineConsumer extends WorkerHost {
  constructor(
    ...
    @InjectQueue(QueueStorage.SEARCH_ENGINE_COLLECTION)
    private searchEngineCollectionQueue: Queue,
  ) {
    super();
  }
  ...
  async process(
    ...
    try {
      ...
      if (error.httpStatus === 404) {
        const schema = this.typesenseMetadataRegistry.getSchemaByName(
          collection.alias,
        );
    ...
    const auroraMetadata = this.auroraMetadataRegistry.getSchemaByAggregateName(
      collection.alias.endsWith('Schema')
        ? collection.alias.slice(0, -6)
        : collection.alias,
    );

    // dynamic import
    const imports = await import(
      `../../../../@app/${auroraMetadata.boundedContextName}/${auroraMetadata.moduleName}`
    );

    // get pagination query
    const paginationQuery = await this.queryBus.ask(
      new imports[
        `${auroraMetadata.boundedContextName.toPascalCase()}Paginate${auroraMetadata.moduleNames.toPascalCase()}Query`
      ](
        {
    ...
    if (paginationQuery.rows.length === 0) {
      ...
      await this.typesense.aliases().upsert(collection.alias, {
