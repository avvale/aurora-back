...
import {
  ...
  SearchEngineDeleteCollectionsCommand,
} from '@app/search-engine/collection';
import {
  SearchEngineCreateFieldsCommand,
  SearchEngineDeleteFieldsCommand,
...
export class SearchEngineTypesenseImplementationService {
  private readonly logger = new Logger(
    SearchEngineTypesenseImplementationService.name,
  );
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    for (const collection of collections) {
      ...
      const alias = aliases.aliases.find(
        (alias) => alias.collection_name === collection.name,
      );
    ...
    await this.commandBus.dispatch(
      new SearchEngineCreateCollectionsCommand(collectionsPayload),
    );

    // create existing queues in redis
    await this.commandBus.dispatch(
      new SearchEngineCreateFieldsCommand(fieldsPayload),
    );
  ...
  async delete(schemaName: string): Promise<void> {
    try {
      ...
      if (error.httpStatus === 404) {
        this.logger.warn(
          `Collection ${schemaName} not found to be deleted in Typesense server`,
        );
      }
    }
