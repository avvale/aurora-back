...
describe('SearchEngineCreateFieldsController', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      controllers: [SearchEngineCreateFieldsController],
      providers: [
        {
    ...
    controller = module.get<SearchEngineCreateFieldsController>(
      SearchEngineCreateFieldsController,
    );
  });
  ...
  describe('main', () => {
    ...
    test('should return an searchEngineMockFieldData created', async () => {
      expect(await controller.main(searchEngineMockFieldData)).toBe(undefined);
    });
  });
});
